...
export default function TeacherCreateTestScreen({ navigation }) {
  const { classes } = React.useContext(TeacherContext);
  ...
  const [correctOption, setCorrectOption] = useState('A'); // Default correct
  ...
  const handleSaveTest = () => {
    if (!selectedClassId) {
      Alert.alert('No Class Selected', 'Please select a class to attach this test to before saving.');
      return;
    }
    if (questions.length === 0) {
      Alert.alert('Empty Test', 'Please add at least one question before saving.');
      return;
    }
  ...
  const renderEditMode = () => (
    <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {classes.length === 0 ? (
          <Text style={styles.emptyClassText}>You need to create a class first from the dashboard.</Text>
      ) : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.classList}>
            {classes.map((cls) => (
                <TouchableOpacity 
                   key={cls.id}
                   style={[styles.classPill, selectedClassId === cls.id && styles.classPillActive]}
                   onPress={() => setSelectedClassId(cls.id)}
                >
                    <Text style={[styles.classPillText, selectedClassId === cls.id && styles.classPillTextActive]}>
                        {cls.name}
                    </Text>
                </TouchableOpacity>
            ))}
          ...
          {['A', 'B', 'C', 'D'].map((opt) => (
            <View key={opt} style={styles.optionRow}>
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  if (opt === 'A') setOptionA(text);
                  else if (opt === 'B') setOptionB(text);
                  else if (opt === 'C') setOptionC(text);
                  else setOptionD(text);
                }}
              />
            </View>
          ))}
      ...
      {questions.map((q, index) => (
        <View key={q.id} style={styles.questionCard}>
          <View style={styles.questionHeader}>
            ...
            <TouchableOpacity onPress={() => handleRemoveQuestion(q.id)} style={styles.eraseBtn}>
  ...
  const renderPreviewMode = () => (
    <ScrollView style={styles.content}>
      <View style={styles.previewHeaderCard}>
         ...
         <Text style={styles.previewSubtitle}>Total Questions: {questions.length}</Text>
      </View>
          ...
          <View style={styles.previewOptionsList}>
            {Object.entries(q.options).map(([key, val]) => (
              <View key={key} style={styles.previewOptionRow}>
                <View style={[styles.previewOptionBubble, q.correctAnswer === key && styles.previewOptionBubbleCorrect]}>
                  <Text style={[styles.previewOptionBubbleText, q.correctAnswer === key && styles.previewOptionBubbleTextCorrect]}>{key}</Text>
                </View>
        ...
        <View style={styles.bottomBar}>
          {isPreviewMode ? (
            <>
              ...
              <TouchableOpacity style={styles.bottomPrimaryBtn} onPress={handleSaveTest}>
            ...
            <>
              <TouchableOpacity
                ...
                onPress={() => {
                  if (questions.length === 0) {
                     Alert.alert('No Questions', 'Add some questions before previewing.');
                     return;
                  }
                  setIsPreviewMode(true);
                }}
              >
                <Ionicons name="eye-outline" size={20} color="#4285F4" style={{ marginRight: 8 }} />
...
const styles = StyleSheet.create({
  ...
  cardTitle: { fontSize: 16, fontWeight: '700', color: '#2D3748', marginBottom: 15 },
  
  emptyClassText: { color: '#E53E3E', fontSize: 14, marginBottom: 15, fontStyle: 'italic' },
  ...
  questionText: { fontSize: 16, fontWeight: '600', color: '#2D3748', flex: 1, marginRight: 10 },
  eraseBtn: {
    padding: 5,
  ...
  previewTitle: { fontSize: 20, fontWeight: '800', color: '#2D3748', marginBottom: 5 },
  ...
  previewQuestionText: { fontSize: 18, fontWeight: '700', color: '#2D3748', marginBottom: 20 },
  ...
  bottomSecondaryBtnText: { color: '#4285F4', fontSize: 16, fontWeight: '700' },
  bottomPrimaryBtn: {
    flex: 1,
    backgroundColor: '#38A169',
    padding: 16,
