 ...
 * Allows teachers to toggle between auto and manual scoring for each student.
 * 
...
export default function TeacherReportsScreen({ navigation, route }: Props) {
  ...
  if (classIdParam) {
    console.log('Loading reports for class:', classIdParam);
  }
  ...
  const toggleScoreType = (id: string) => {
  ...
  const updateManualScore = (id: string, score: string) => {
    setStudents((currentStudents) =>
      currentStudents.map((student) =>
  ...
  const renderStudentMenu = ({ item }: { item: StudentReport }) => {
    const finalScore = item.useAuto
      ? item.autoScore
      : item.manualScore || '-';
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardHeaderText}>
            ...
            <Text
              style={[
                styles.statusText,
                { color: item.submitted ? '#34A853' : '#EA4335' },
              ]}
            >
          ...
          <View style={styles.qualificationSection}>
            ...
            <View style={styles.scoreRow}>
              ...
              <View
                style={[
                  styles.scoreBox,
                  !item.useAuto && styles.activeScoreBox,
                ]}
              >
                ...
                <TextInput
                  style={[
                    styles.manualInput,
                    item.useAuto && styles.disabledInput,
                  ]}
  ...
  const keyExtractor = (item: StudentReport) => item.id;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.infoBanner}>
        ...
        <Text style={styles.bannerText}>
          Review auto-calculated scores or enter manual qualifications for your
          class.
        </Text>
      </View>
